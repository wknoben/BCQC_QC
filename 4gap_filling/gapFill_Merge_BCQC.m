    ...
    for i=1:length(ID) %iterate through the ID values in this file
        ...
        if exist(file, 'file') == 2     %check if this file exists
            ...
            if (sum(isnan(data_stngg))<730)     % if there are nans, but they don't take up the entire time-series 
                ...
                LLE(i,1:4)=LLEgg(1,1:4);  % Make an LLE VAR
            end           
        end      
    end
end 

ID=LLE(:,4); % calculate the per Create ID variable
Nanpi=(Nanp/638)*100; % calculate the percentage of missing data points 
percent=20; %threashold for missing data
...
ID1=ID;
LLE(threashold,:)=[]; % remove numbers form LLE that exceed the missing number threashold
LLE1=LLE;
LLES(threashold,:)=[]; % remove numbers form LLE that exceed the missing number threashold
LLES1=LLES;
...
for i=1:length(ID) % iterate through the ID values in this file, (the first file didn't work, due to bad data)
    ...
    file=strcat(num2str(ts(i)),'.mat'); % create a string corresponding to the name of ML output files
