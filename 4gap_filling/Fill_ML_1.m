...
%0 Basic inputs
% % Plato
FileGauge='/Users/cjh458/Desktop/BCQC_QC/3unify_data/ghcnd_UNI/AllGauge_QC_bcqc_noflags.nc4';
% FileGauge='/Users/cjh458/Desktop/Guoquiang_WF/4unify_data/ghcnd_UNI/AllGauge_QC_1.nc4';
% output files
Outpath='/Users/cjh458/Desktop/BCQC_QC/4GapFill_forChristian/';

%1 Basic settings
Validnum.all=1;  % the least number of valid samples for each station and each variable to be included in the gap filling process
...
% function [IDne_num,IDne_dist]=f_findnear(FileGauge,varvv,radius,leastne,Validnum,filevv,CCreavv)
% numGauge=20;        %number of neighboring gauges to include.
% if exist(filevv,'file')
...
%     disi(:,1)=IDnum;
%     for j=1:gnum
%         disi(j,2)=f_lldistkm(lat(i),lon(i),lat(j),lon(j));
%     end
%     disi=sortrows(disi,2);
%     IDne_num(i,1:numGauge)=disi(2:(numGauge+1),1);
%     IDne_dist(i,1:numGauge)=disi(2:(numGauge+1),2);
%     
% end
% 
...
% %     end    
% % end
% save(filevv,'IDne_num','IDne_dist');
% 
% end
