...
% data--[date in yyyymmdd, precipitation (mm), snowfall (mm), Tmin (C), Tmax (C), Tmean (C)]
% lle--[latitude, longitude, elevation (m)]
% ID--gauge ID
% source--GHCN-D
% varname--[date,precipitation(mm),snowfall(mm),Tmin(C),Tmax(C),Tmean(C)]
...
% load('/home/cjh458/ghcnd_ST/GaugeValid1.mat'); 
...
%     'AWND',... % Weather Type where ** has one of the following values
%     'AWDR',... % Weather Type where ** has one of the following values
    };

Gnum=length(GaugeValid.ID);
for i=1:Gnum
    ...
    if ~exist(Outfile,'file')
        if ~exist(Infile,'file')
            ...
            % save file
            f_savemat(Outfile,data,mqsflag,lle,ID,varname,source);
        end
    end
...
function f_savemat(Outfile,data,mqsflag,lle,ID,varname,source)
